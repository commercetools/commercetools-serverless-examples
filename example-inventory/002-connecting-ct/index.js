  ...
  const client = createClient({
    middlewares: [
      ...
      createAuthMiddlewareForClientCredentialsFlow({
        ...
        scopes: config.CT_SCOPES.indexOf(' ') > 0 ? config.CT_SCOPES.split(' ') : [config.CT_SCOPES],
      }),
...
exports.helloWorld = function helloWorld(req, res) {
  ...
  if (req.body.sku === undefined) {
    ...
    const inventoryUri = requestBuilder.inventory.where(`sku="${req.body.sku}"`).build()
    const inventoryRequest = {
      method: "GET",
      uri: inventoryUri
    }
    const client =createCTClient()
    client.execute(inventoryRequest).then(result => {
      const results = result.body.results
      if (!results.length) {
        return res.status(404).json({error:'Inventory for SKU not found!'})
      }
